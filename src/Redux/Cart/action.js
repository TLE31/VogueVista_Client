...
const getCartItems = (userId) => (dispatch) => {
  ...
  return axios(`https://voguevista-server.onrender.com/cart/${userId}`, {
    headers: {
...
const postCartItems = (item, userId) => (dispatch) => {
  ...
  return axios
    .post(`https://voguevista-server.onrender.com/cart/${userId}`, item)
    .then((res) => {
...
const removeCartItem = (userId, productId, toast) => (dispatch) => {
  ...
  return axios
    .delete(
      `https://voguevista-server.onrender.com/cart/${userId}/${productId}`
    )
    .then((res) => {
